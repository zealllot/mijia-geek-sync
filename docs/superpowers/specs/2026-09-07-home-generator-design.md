...
把 1302 的实际情况反推成 `yang-home/homes/1302.toml`，结果：

- **七个区的闸门链全部对上**，例外只有两行（电梯口、电梯玄关的 `master = false`）
- 传感器、目标（含 `did:siid` 形式的开关键）、手动锁的开关键、本地/全局阈值、
  关灯延时都能从 toml 一一对上
- 全局照度那条腿是 `deviceGet` 读全屋参考传感器（`blt.3.1q3q…b84c01`），
  不是读 `luxQuanJu` 变量 —— `[house] globalLux` 表达得了
- 动作词汇从四个补到五个（见上）

还没验的：户级那八条规则的参数化、场景的触发源写法、`at` 行列换算成像素。
