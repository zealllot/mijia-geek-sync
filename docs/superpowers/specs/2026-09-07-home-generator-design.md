...
night  = true                        # 分昼夜：夜间用夜灯参数。目标全是哑灯时自动为假
lux    = 150                         # 本地照度阈值，初值
...
**闸门链能推的都推，只写例外。** 拿 1302 的七个区验过，推导规则是：

| 闸门 | 从哪推出来 |
|---|---|
| `ziDongHua=1` | 默认每个区都有；`master = false` 才退出（1302 只有电梯那两处） |
| `shouDong<区>=0` | 这个区声明了 `switches`，且 `[house] manualLock` 开着 |
| `<场景变量>=0` | `[[scene]] zone` 指到这个区 |
| `huiKe=0` | 这个区在 `guestMode.zones` 里 |

**七个区推出来的闸门链跟现有 43 条规则完全一致**，整份 toml 里只写了两行例外。
关灯规则的闸门 = 开灯闸门 − 手动锁 − 昼夜分支，是规律，不用配。
「本区灯组已灭」那道判断用的就是第一个目标的 did，也不用配。

**动作词汇五个**：关灯 / 开灯 / 开窗帘 / 关窗帘 / 清手动锁。
「开灯」带可选的 `bri`、`cct`：不给就用全局参数，给变量名就用夜灯参数，
给数字就是字面值（1302 观影把某盏调到 3%）。
**清手动锁**是拆场景时必需的 —— 进入睡眠/观影会把该区手动锁清掉，
不然场景结束之后那个区的感应就失灵了；1302 三条场景规则里都有这一步。
...
两户的实际情况都反推成了 toml（`yang-home/homes/1302.toml`、`1301.toml`），
**13 个区的闸门链全部对上，零误差**。两户正好压住形状的两端：

| | 1302 | 1301 |
|---|---|---|
| 照度 | 双阈值（本地 或 全屋参考） | 单阈值 |
| 手动锁 | 七个区都有 | 没有 |
| 场景 | 睡眠 ×2、观影 | 没有 |
| 光亮灯灭 | 七条 | 没有 |
| 会客 | 只压进门区 | 压五个区，洗衣房除外 |
| 例外行数 | 2（电梯两处 `master = false`） | 1（厨房 `night = false`） |

反推逼出来的四处修正：

1. **总开关是每个区各自的，不是全屋统一的** —— 1302 的电梯口和电梯玄关不受它管。
   原来的模型默认它是全局的。
2. **全局照度那条腿是 `deviceGet` 读传感器**（`blt.3.1q3q…b84c01`），
   不是读 `luxQuanJu` 变量。`[house] globalLux` 表达得了，但先前的理解是错的。
3. **动作词汇漏了「清手动锁」** —— 进入睡眠/观影要把该区手动锁清成 0，
   不然场景结束后那个区的感应就失灵了。1302 三条场景规则里都有这一步。
4. **「分不分昼夜」是配置，不全是设备能力** —— 1301 的厨房灯可调（piid 2/3 都写了），
   但规则里只写全局参数、没有夜灯分支。目标全是哑灯继电器时自动没有分支（1302 电梯两处），
   而厨房这种是有意选的，得有 `night = false` 表达。

还没验的：户级那八条规则的参数化、场景触发源的写法、`at` 行列换算成像素。

## 差异不等于缺陷

回归比对出来的差异有两种，读法完全不同：

- **生成器错了** —— 少一道闸门、接线接错、上下界写宽了。这种要修。
- **这户的习惯** —— 1301 的厨房灯可调，但住户就是不要夜间调暗，规则里只写全局参数。
  这种不要「修」，要给它一个逃生口（`night = false`），然后**在配置里留着**。

第二种比想象的多，而且会越来越多：越往后配的户越多，越会撞上「这家老人多不要手动锁」
「那家厨房不分昼夜」「另一家会客只压客厅」这类东西。
把它们规范化掉是错的 —— 那等于替住户决定他家该怎么用灯。

所以生成器的形状是**默认 + 逃生口**，而不是**唯一正确的形状**。
「能推的推、只写例外」这条之所以成立，前提就是例外被当成头等公民：
配置里那三行例外（1302 电梯两处的 `master = false`、1301 厨房的 `night = false`）
不是妥协，是这套东西正常工作的样子。

**验收标准因此不是「diff 为零」**，而是「每一处 diff 都能说清是哪一种」。
